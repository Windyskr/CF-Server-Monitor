...
export async function initializeMissingTrafficSnapshots(
  ...
  for (const server of servers || []) {
    ...
    const interfaces = normalizeTrafficInterfaces(metrics.network_interfaces);
    let changed = false;
    for (const type of types) {
      ...
      try {
        ...
        for (let index = 0; index < types.length; index += 1) {
          ...
          snapshots[type] = {
            ...
            ...(Object.keys(normalizeTrafficInterfaces(source.network_interfaces)).length > 0
              ? { interfaces: normalizeTrafficInterfaces(source.network_interfaces) }
              : {})
          };
...
export async function checkTrafficReports(db, options = {}) {
  ...
  if (options.staggered && zonedParts) {
    ...
    const slotMinutes = isSundayRotationWindow
      ? { daily: 5, weekly: 6, cycle: 7 }
      : { daily: 0, weekly: 1, cycle: 2 };
  ...
  if (acceptsCycleReports && cycleServers.size > 0 && (!options.staggered || Number(zonedParts?.minute) >= (isSundayRotationWindow ? 7 : 2))) {
    reportTypes.push('cycle');
  }
  reportTypes = Array.from(new Set(reportTypes)).filter(type => type !== 'cycle' || cycleServers.size > 0);
  ...
  try {
    ...
    const reportPayloads = {
      ...
      weekly: claimedReportTypes.includes('weekly')
        ...
        : [],
      cycle: claimedReportTypes.includes('cycle')
        ? buildTrafficReportPayloads(servers, usageRows.cycle, '周期')
        : []
    };
